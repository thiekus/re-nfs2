...
int* _glbptr_gGameSetupIsUnstable = (void*)0x004da82c;
...
void DEFAULT_CALL Nfs2_SystemNLibStartUp(void)
{
...
void DEFAULT_CALL Nfs2_SystemNLibCleanUp(void)
{
...
void DEFAULT_CALL Nfs2_BefuddleCode(void)
{
...
void DEFAULT_CALL Nfs2_ChangeExtension(char* ext)
{
...
void DEFAULT_CALL Nfs2_ResetGame(void)
{
...
void DEFAULT_CALL Nfs2_CleanUpGameModule(void)
{
...
void DEFAULT_CALL main(int argc, char* argv[])
{
    ...
    while (nResult != 0) {
        ...
        if (nResult != 0) {
            setup = MinFront_ParseOptions(argc, argv);
            setup = Front_BuildStream(setup);
            *_glbptr_gGameSetupIsUnstable = 0;
            Nfs2_GameModuleStartUp(setup);
            if (!*_glbptr_gGameSetupIsUnstable == 0) {
                // while (simVar != 0) {
                //     simVar = 0;
                //     Nfs2_ResetGame();
